...
export default function TopLocationScreenDetails({ navigation, route }) {
    ...
    const fetchTopLocationDetails = useCallback(async (country, page = 1, filters = {}) => {
        try {
            setLoading(true);
    ...
    useEffect(() => {
        if (pageno > 1 && userInfo?.country) {
            fetchTopLocationDetails(userInfo.country, pageno);
        }
    }, [pageno]);
    ...
    const handleWishlist = async (packageId) => {
        try {
            ...
            if (response.data.response == true) {
                ...
                fetchTopLocationDetails(userInfo?.country, 1);
            }
        } catch (error) {
    ...
    const resetFilters = async () => {
        try {
            ...
            await fetchTopLocationDetails(userInfo?.country, 1);
